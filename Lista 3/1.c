/*
(Cadastro de Pessoa) Crie um programa em C de cadastro de no máximo 20pessoas, recebendo as seguintes características:
nome (tamanho máximo de 30 caracteres), sexo (0 para masculino e 1 para feminino) e data de nascimento(“DD/MM/AAAA”). 
O programa deve a todo momento disponibilizar um menu de operação com as seguintes opções:
1 - Cadastrar pessoa
2 - Excluir pessoa
...
6 - Sair do programa
Na opção de número 1, o programa deve receber as características de uma pessoa, certificando de que os dados de sexo e 
nascimento respeitem o padrão solicitado (especialmente o padrão de data, que deve ser recebido como uma única string).
Já na opção de número 2, excluir pessoa, deve-se receber um nome de pessoa e checar se ela existe no sistema. Caso exista, 
esta deve ser excluída. Caso contrário, deve-se exibir uma mensagem de erro, informando que esta pessoa ainda não foi cadastrada.
Nas opções 3, 4 e 5 os dados a serem exibidos de uma pessoa são: nome, sexo (“masculino” ou “feminino”) e data de nascimento
(DD/MM/AAAA).Por fim, na opção 6. o programa deve ser encerrado.
*/
