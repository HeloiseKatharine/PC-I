/*Altere o programa da lista 3 (cadastro de pessoas - questão 1) de forma que
ao digitar a opção 6 (“sair do programa”) os dados de pessoas sejam salvos
em um arquivo binário chamado pessoas. Além disso, caso o arquivo
pessoas exista, deve-se carregar as informações das pessoas existentes
nesse arquivo. Desse modo, os dados cadastrados não se perderão ao
...
void cadastro_pessoa (struct cadastro pessoa[quant_pes], struct dma data[quant_pes], int *cont_cadastro)//fun��o cadastro
{
...
void excluir(struct cadastro pessoa[quant_pes], int *cont_cadastro)// Fun��o Excluir pessoa
{
...
void exibe( struct cadastro pessoa[quant_pes], int *cont_cadastro)// Fun��o exibir dados de todas as pessoas
{
    int  i;
...
void salva_arquivos( struct cadastro pessoa[quant_pes], int *cont_cadastro, FILE *pont_arq)// Fun��o exibir dados de todas as pessoas
{
    int  i;

    for(i=0; i < *cont_cadastro; i++)
    {
        fprintf(pont_arq, "Nome: %s ", pessoa[i].nome);
        fprintf(pont_arq, "Sexo: %d ", pessoa[i].sexo);
        fprintf(pont_arq, "Data: %d/", pessoa[i].data.dia);
        fprintf(pont_arq, "%d/", pessoa[i].data.mes);
        fprintf(pont_arq, "%d", pessoa[i].data.ano);
    }
}
...
//    struct cadastro pessoa[quant_pes];//tipo pessoa
        ...
        while( num != 6 )
        {
            ...
            case 6://sair do programa
                    ...
                    //ABRINDO O ARQUIVO 2.TXT COM TIPO DE ABERTURA WB
                    pont_arq = fopen("pessoas.txt","wb");

                    salva_arquivos(pessoa, &cont_cadastro, pont_arq );// Fun��o salvar dados de todas as pessoas
